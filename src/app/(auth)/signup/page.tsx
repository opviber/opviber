...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        // If user is auto-confirmed or needs email confirmation
        if (data.session) {
  ...
  const handleOAuth = async (provider: "google" | "github") => {
    ...
    try {
      ...
      setErrorMsg(e.message || `Failed to sign up with ${provider}`);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="space-y-4">
      <form onSubmit={handleSignup} className="space-y-4">
        <div className="space-y-1 text-center">
          ...
          <p className="text-xs text-zinc-550">Start building and cloning web applications in minutes</p>
        </div>
        ...
        {successMsg && (
          <div className="p-3 bg-emerald-600/10 border border-emerald-500/20 rounded text-emerald-400 text-xs font-semibold text-center">
            {successMsg}
          </div>
        )}
